// v0.4 - registro de incidentes (tirones) y percentil 99.
// La media miente sobre como se siente algo: lo que recuerdas
// son los picos, no los 132 segundos buenos.
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            egui::ScrollArea::vertical().show(ui, |ui| {
                ...
                match &s.peer {
                    ...
                    Some(p) => {
                        ui.label(format!("Rival: {}   ({:.1} min)", p.ip, p.minutos));
                        ui.add_space(4.0);

                        // --- INCIDENTES: lo que de verdad se siente ---
                        let (ci, txt_inc) = calidad_incidentes(p.inc_por_min, p.minutos);
                        ui.horizontal(|ui| {
                            ui.label(
                                egui::RichText::new(format!("{}", p.total_incidentes))
                                    .color(ci)
                                    .strong()
                                    .size(38.0),
                            );
                        ...
                        if p.peor_frames > 0 {
                            ui.label(
                                egui::RichText::new(format!(
                                    "peor: {} frames perdidos",
                                    p.peor_frames
                                ))
                                .color(if p.peor_frames >= 6 {
                        ...
                        // --- Lista de tirones recientes ---
                        if !p.incidentes.is_empty() {
                            ui.label(egui::RichText::new("Ultimos tirones").strong());
                            egui::Grid::new("inc").num_columns(3).striped(true).show(
                                ui,
                                |ui| {
                                    for i in &p.incidentes {
                                        let col = if i.frames >= 6 {
                                            egui::Color32::from_rgb(235, 110, 100)
                                        } else {
                                            egui::Color32::from_rgb(230, 200, 100)
                                        };
                                        ui.colored_label(col, format!("hace {:.0}s", i.hace_s));
                                        ui.colored_label(col, format!("{:.0} ms", i.ms));
                                        ui.colored_label(col, format!("{} frames", i.frames));
                                        ui.end_row();
                                    }
                                },
                            );
                        ...
                        egui::Grid::new("stats").num_columns(2).show(ui, |ui| {
                            ...
                            ui.colored_label(
                                cj,
                                format!("{:.1} ms  ({})", p.jitter_ms, etiqueta),
                            );
                            ui.end_row();
                            ...
                            ui.label("P99");
                            ui.colored_label(
                                if p.p99_ms > p.mean_ms * 2.5 {
                                    egui::Color32::from_rgb(235, 110, 100)
                                } else {
                                    egui::Color32::GRAY
                                },
                                format!("{:.1} ms", p.p99_ms),
                            );
                            ui.end_row();
                        ...
                        ui.add_space(4.0);
                        ui.weak("P99 muy por encima del intervalo = inestable,");
                        ui.weak("aunque la media parezca buena.");
                    }
                }
                ...
                let mut laxo = s.laxo;
                if ui
                    .checkbox(&mut laxo, "Modo laxo (acepta cualquier flujo)")
                    .changed()
                {
                    self.estado.lock().unwrap().laxo = laxo;
                }
                ui.weak("Solo para depurar.");
...
fn calidad_incidentes(por_min: f64, minutos: f64) -> (egui::Color32, &'static str) {
    if minutos < 0.2 {
        return (egui::Color32::GRAY, "midiendo");
    }
    if por_min < 0.5 {
        (egui::Color32::from_rgb(90, 220, 110), "limpio")
    } else if por_min < 2.0 {
        (egui::Color32::from_rgb(230, 200, 100), "algun tiron")
    } else if por_min < 6.0 {
        (egui::Color32::from_rgb(240, 150, 90), "molesto")
    } else {
        (egui::Color32::from_rgb(235, 110, 100), "injugable")
    }
}
...
fn grafico(ui: &mut egui::Ui, hist: &[f32], media: f64) {
    ...
    let techo = ((media * 3.0) as f32).max(50.0);
    ...
    // Linea del umbral de tiron (50 ms)
    let y_umbral = r.bottom() - (50.0 / techo).clamp(0.0, 1.0) * r.height();
    pintor.line_segment(
        [egui::pos2(r.left(), y_umbral), egui::pos2(r.right(), y_umbral)],
        egui::Stroke::new(1.0, egui::Color32::from_rgba_unmultiplied(235, 110, 100, 110)),
    );
    ...
    for (i, v) in hist.iter().enumerate() {
        ...
        let col = if *v > 50.0 {
            egui::Color32::from_rgb(235, 110, 100)
        } else if (*v as f64 - media).abs() < media * 0.25 {
